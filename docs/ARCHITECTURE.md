...
With the optional media lane enabled, step 2 additionally queries the top-30 non-text media chunks independently of the text vector window. A candidate is admitted only when its raw cosine distance is at or below the `[search]` gate for its media type. Up to `floor(limit * media_lane_fraction)` absent candidates are inserted at evenly spaced result positions, displacing only the tail; media already in the RRF result retains its position. Image and PDF display scores use distinct linear distance maps so their relevance thresholds remain calibrated independently. Audio/video currently use image thresholds and endpoints until calibrated.
