...
- **Embedder** (`src/embed/`): provider/model registry (`src/embed/registry.rs`) resolves `(EmbedProvider, model, dim)` and validates against a static table. Gemini `gemini-embedding-2` (default, text+image+PDF, native 3072, Matryoshka to 768/1536, no `taskType`), `gemini-embedding-001` (text only, `RETRIEVAL_DOCUMENT`/`RETRIEVAL_QUERY`), Voyage `voyage-multimodal-3.5` (text+image+PDF, native 1024), and Voyage `voyage-4` family (text only, native 1024, `document`/`query`). `Fake` embedder for tests (SHA256-seeded, L2-normalized, media-capable). `AnyEmbedder` enum for static dispatch. Embedding inputs are typed: `EmbedInput::Text(String)` for text, `EmbedInput::Media(Vec<MediaPart>)` for binary parts — bytes are never logged.
...
- **Media preparation** (`src/media_prepare.rs`): byte-based image/PDF detection. Images: PNG/JPEG retained as-is; GIF/WebP normalized to PNG from first frame (`truncated=true` when animated); pixels capped at 16 M (downscaled). PDFs: `PdfMode::Native` (Gemini 2) extracts page subsets via `DocumentEditor::extract_pages_to_bytes`; `PdfMode::Raster` (Voyage multimodal) renders each page to PNG at configured DPI. Default pages-per-chunk is 1 (Google quality recommendation). Cache keys are length-framed over all provider parts to prevent partition ambiguity.
- **Indexer** (`src/indexer/`): single pipeline consuming **relative** dirty paths from an `mpsc::UnboundedReceiver<PathBuf>`. `reindex_one` reads and prepares all content before any store mutation; a preparation/embed failure leaves prior indexed state intact. Resolves cache hits for all typed inputs, then commits chunks+vectors+cache entries+file state in one SQLite transaction via `Store::replace_file`. Deletion (`Store::remove_file`) is also a single transaction.
...
- **Search** (`src/search/`): hybrid BM25+semantic retrieval with Reciprocal Rank Fusion (k=60, candidate pool 30 per ranker). Media chunks contribute to the vector branch only (not FTS). Default `/search` is mixed text+media. With `[search].media_lane_enabled`, blended search independently fetches `max(30, limit)` all-media candidates, gates them by type-specific raw cosine distance, and inserts up to the configured fraction of previously absent candidates at evenly spaced positions; unused slots remain text. `media_only=true` queries non-text chunks and densely reranks; with the lane enabled its display score uses the same type-specific distance map. Optional typed `media_types` restricts that corpus to image, PDF, audio, or video. `/similar` works for media paths (stored vectors); FTS bag uses text chunks only. Media snippets are generated at hydration: `Image`, `PDF page N`, `PDF pages start–end`. Pure `fuse_rrf` is unit-tested.
- **API** (`src/api/`): axum 0.8 router with public `/health`: requests without a valid bearer receive `{ok, authenticated: false}` liveness only, while valid bearers receive index detail and `authenticated: true`. `/search` + `/similar` are bearer-protected via middleware. `/search` accepts optional `media_only: bool` and, only with `media_only=true`, `media_types: [image|pdf|audio|video]`. Structured errors as `{error, code}` JSON. `AppState` is `Send+Sync` (statically asserted in `server.rs`).
...
4. Fingerprint mismatches, no `--reembed`: refuses to start with an error naming every changed field and both values,    e.g. `index built with provider=gemini model=gemini-embedding-2 dim=3072, config says provider=voyage model=voyage-4 dim=1024; re-embed required: run with --reembed`.
...
- `meta` — `schema_version=3`, `path_schema_version=1` (after the absolute→relative path migration), `embedding_provider`, `embedding_model`, `embedding_dim` (the index fingerprint), `last_full_scan`.
...
With the optional media lane enabled, step 2 additionally queries `max(30, limit)` non-text media chunks independently of the text vector window. A candidate is admitted only when its raw cosine distance is at or below the `[search]` gate for its media type. Up to `floor(limit * media_lane_fraction)` absent candidates are inserted at evenly spaced result positions. A full result displaces the last text entry per admission; an underfilled result uses its free capacity first. Media already in the RRF result is never duplicated and never displaced; admission stops once no text entry remains, so the result never exceeds `limit` and never loses media to truncation. Insertion still shifts later entries down one index. Image and PDF display scores use distinct linear distance maps so their relevance thresholds remain calibrated independently. PDF thresholds and endpoints are provisional, calibrated on five files. Audio/video currently reuse image thresholds and endpoints until calibrated. Enabling the lane switches media display scores to the distance map for `/search`, both blended and `media_only`, including hits the blended search already returned. `/similar` is not part of the lane and keeps rank-derived media scores.
...
- `/health` is always reachable and returns `200 OK`; an unauthenticated response is liveness only and must not be used to verify credentials. `/search` and `/similar` are bearer-gated. Comparison is constant-time.
