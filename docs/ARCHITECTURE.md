...
With the optional media lane enabled, step 2 additionally queries `max(30, limit)` non-text media chunks independently of the text vector window. A candidate is admitted only when its raw cosine distance is at or below the `[search]` gate for its media type. Up to `floor(limit * media_lane_fraction)` absent candidates are inserted at evenly spaced result positions, each displacing the last text entry. Media already in the RRF result is never duplicated and never displaced; admission stops once no text entry remains, so the result never exceeds `limit` and never loses media to truncation. Insertion still shifts later entries down one index. Image and PDF display scores use distinct linear distance maps so their relevance thresholds remain calibrated independently. PDF thresholds and endpoints are provisional, calibrated on five files. Audio/video currently reuse image thresholds and endpoints until calibrated. Enabling the lane also switches every media display score to the distance map, including hits the blended search already returned.
